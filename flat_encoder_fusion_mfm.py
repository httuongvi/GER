...
MODEL_PATH = os.path.join(OUTPUT_DIR, "vilt_ablation_pure_vision_baseline.pth")
...
# MFM Config — GIỮ NGUYÊN từ baseline
MASK_PROB_FACE   = 0.3
...
class ViLTWithMFM(nn.Module):
    ...
    def forward(self, face_feats, face_mask, object_feats, object_mask,
        ...
        # Xóa: Averaged Text, Decoder
        # Thêm: score_head áp trực tiếp lên global_visual_feat → (B, NUM_CLASSES)
...
if __name__ == "__main__":
    ...
    # ---- 1. Data (RAM cache inside __init__) -----------------------------
    ...
    # ---- 2. Model --------------------------------------------------------
    print(f"\nInitializing ViLTWithMFM (Ablation: Pure Vision Baseline)...")
    # [ABL] Khởi tạo không cần semantic_path
    ...
    # ---- 3. Losses -------------------------------------------------------
    class_weights = torch.tensor([1.0, 1.2, 1.5]).to(DEVICE)  # Pos / Neg / Neutral
    criterion_cls = FocalLoss(alpha=class_weights, gamma=2.0, label_smoothing=0.1)
    criterion_mfm = nn.MSELoss()   # [ABL] MSE truyền thống thay vì InfoNCE
    # [ABL] Xóa criterion_sam
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # [ABL] Bỏ criterion_sam khỏi lệnh gọi train_epoch
    ...
    test_loss, test_acc, preds, labels_test = eval_epoch(
